...
export const COMMANDS = [
    ...
    },
    {
        ...
        async run(env) {
            ...
            pickOne(env.tui, `思考强度 · ${sel.model}`, [
                ...
                // 静默生效:状态栏模型段的强度后缀即反馈。
                const choice = { ...sel, reasoningEffort: effort === "" ? undefined : effort };
...
async function pickModel(env, provider) {
    ...
    pickOne(env.tui, `选择模型 · ${provider}`, models.map((m) => ({ value: `${provider}\u0000${m.id}`, label: m.name, description: m.id })), async (value) => {
        ...
        const efforts = meta.reasoningEfforts ?? [];
        const pickEffort = () => pickOne(env.tui, `推理强度 · ${model}`, [{ value: "", label: "默认", description: env.controller.selection?.reasoningEffort ? undefined : "· 当前" },
            ...efforts.map((e) => ({ value: e.id, label: e.name, description: env.controller.selection?.reasoningEffort === e.id ? "· 当前" : undefined }))], (effort) => {
