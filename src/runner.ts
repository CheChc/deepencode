...
const editorTheme = {
  ...
  selectList: {
    ...
    description: (s: string) => theme.faint(s),
    scrollInfo: (s: string) => theme.faint(s),
    noMatch: (s: string) => theme.faint(s),
  },
};
...
export class TuiRuntime {
  ...
  private answerQuestions(request: AskUserQuestionRequest): Promise<{ answers: Array<{ id: string; selected: string[]; custom?: string }> }> {
    return new Promise((resolve) => {
      ...
      const options: Array<{ value: string; label: string; description?: string }> = (question.options ?? []).map((o) => ({
        value: o.label,
        label: o.label,
